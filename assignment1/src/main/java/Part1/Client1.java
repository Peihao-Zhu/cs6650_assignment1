...
public class Client1 {
    ...
    public Client1(Argument argument, String url, CountDownLatch totalThreads, ResultData resultData) {
    ...
    public void testSingleRequestLatency() throws InterruptedException {
        ...
        MyThread1 thread = new MyThread1(1, 20000, 1, 90, 10000, argument.getNumLifts(), url, count, totalThreads, resultData, 1);
        thread.start();
        count.await();
    }
    ...
    public void executePhase1() throws InterruptedException {
        ...
        for(int i = 0; i < launchThreads; i++) {
            ...
            MyThread1 thread = new MyThread1(startSkierId, endSkierId, 1, 90, requestPerThread, argument.getNumLifts(), url, count, totalThreads, resultData, 1);
            thread.start();
        }
        count.await();
    }
    public  void executePhase2() throws InterruptedException {
        ...
        for(int i = 0; i < launchThreads; i++) {
            ...
            MyThread1 thread = new MyThread1(startSkierId, endSkierId, 91, 360, requestPerThread, argument.getNumLifts(), url, count, totalThreads, resultData, 2);
    ...
    public  void executePhase3() throws InterruptedException {
        ...
        for(int i = 0; i < launchThreads; i++) {
            ...
            MyThread1 thread = new MyThread1(startSkierId, endSkierId, 361, 420, requestPerThread, argument.getNumLifts(), url, count, totalThreads, resultData, 3);
            new Thread(thread).start();
        }
        count.await();
    }
